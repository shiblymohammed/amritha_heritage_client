...
const ROOM_BOOKING_SERVICE_ID = import.meta.env
  .VITE_ROOM_BOOKING_EMAILJS_SERVICE_ID;
const ROOM_BOOKING_TEMPLATE_ID = import.meta.env
  .VITE_ROOM_BOOKING_EMAILJS_TEMPLATE_ID;
const ROOM_BOOKING_PUBLIC_KEY = import.meta.env
  .VITE_ROOM_BOOKING_EMAILJS_PUBLIC_KEY;
...
export const sendRoomBookingNotification = async (
  ...
  try {
    ...
    if (!ROOM_BOOKING_SERVICE_ID) {
      throw new Error(
        "Room Booking Service ID is missing. Please check VITE_ROOM_BOOKING_EMAILJS_SERVICE_ID in .env file"
      );
    }

    if (!ROOM_BOOKING_TEMPLATE_ID) {
      throw new Error(
        "Room Booking Template ID is missing. Please check VITE_ROOM_BOOKING_EMAILJS_TEMPLATE_ID in .env file"
      );
    }

    if (!ROOM_BOOKING_PUBLIC_KEY) {
      throw new Error(
        "Room Booking Public Key is missing. Please check VITE_ROOM_BOOKING_EMAILJS_PUBLIC_KEY in .env file"
      );
    }
    ...
    const templateParams = {
      to_email: "info@amrithaheritage.com", // Room booking admin email
    ...
    if (error.status) {
      switch (error.status) {
        ...
        case 422:
          console.error(
            "❌ Template Error: Check template variables match exactly"
          );
        ...
        default:
          ...
          alert(
            "❌ Room booking email failed: " + (error.text || error.message)
          );
      }
    } else {
