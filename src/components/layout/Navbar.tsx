...
const NavBar: React.FC = () => {
  ...
  const logoSrc = (hasScrolled && !isMenuOpen) ? '/logoBlack.png' : '/logoWhite.png';
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-400 ease-out ${getNavClass()} ${isNavVisible ? 'translate-y-0' : '-translate-y-full'}`}>
          ...
          <div className="relative flex items-center justify-between h-20 md:h-24">
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
              <a href="/" aria-label="Go to Homepage">
                <img 
                  ...
                  onError={(e) => { e.currentTarget.src = 'https://placehold.co/240x60/3A4A3E/FBF9F6?text=Amritha+Heritage&font=cinzel'; }}
                />
              </a>
            </div>
            ...
            <a 
              ...
              className={`shadow-soft-sunlight hover:shadow-golden-glow-sm transform hover:scale-105 text-xs px-4 py-2 md:text-sm md:px-6 md:py-2.5 animate-float ${
                hasScrolled ? 'btn btn-primary' : 'btn bg-transparent border-2 border-white text-white hover:bg-white/10'
              }`}
            >
              Book Now
            </a>
      ...
      >
        <div className="container mx-auto px-6 lg:px-8 h-full grid grid-cols-1 lg:grid-cols-2 items-center">
          ...
          <nav className="flex flex-col items-center lg:items-start">
            {navLinks.map((link, index) => (
              <a
                key={link.name}
                href={link.href}
                className={`font-playfair text-2xl md:text-4xl lg:text-5xl my-1 md:my-2 leading-tight text-text-on-color transition-all duration-300 ease-out hover:text-white hover:tracking-wide ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          ...
          <div className={`hidden lg:flex flex-col items-start text-left transition-opacity duration-500 ease-out ${isMenuOpen ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: '400ms' }}>
